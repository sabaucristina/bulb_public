...
private extension TaskCellView {
    ...
    func makeSelectionAccessory() -> UIButton {
        ...
        let image = Image.circle?.withTintColor(Color.darkMatteGreen)
    ...
    func makeSnoozeAccessory() -> UIButton {
        ...
        let image = Image.snooze?.withTintColor(Color.darkMatteGreen)
    ...
    func makeCompletedTaskImageView() -> UIImageView {
        let imageView = UIImageView(
            frame: CGRect(
                x: 0,
                y: 0,
                width: ImageSize.iconSize,
                height: ImageSize.iconSize
            )
        )
        imageView.translatesAutoresizingMaskIntoConstraints = false
        imageView.image = Image.checkmarkFill
